...
/**
 * An open addressing hash table data structure that, by default, uses linear
 * probing to handle collisions. The hash function uses the built-in hashCode
 * method and the multiply-add-and-divide method. The load factor kept less than
 * or equal to 0.5. When the load factor reaches 0.5, the entries are rehashed
 * into a new bucket array with twice the capacity.
 *
...
public class OAHashDictionary<K, V> implements Dictionary<K, V> {
     ...
     * @param probingMethod
     */
    public OAHashDictionary(ProbingMethod probingMethod) {
     ...
     * @param probingMethod
     */
    public OAHashDictionary(int cap, ProbingMethod probingMethod) {
        this.n = 0;
